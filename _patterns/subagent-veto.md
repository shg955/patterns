...
detect: >-
  검토자가 지시를 반박한 횟수를 세라. 0 이면 검토 단계가 통과 도장을 찍고 있을 뿐이다.
  특히 리드가 준 전제 자체를 부정한 건수가 0 이면 그 검토는 값이 없다.
...
## 어떻게 적용하나
1. **거부권을 명시적으로 준다.** "검토해라" 가 아니라 **"내가 준 지시 중 틀린 것을 찾아라"**
   라고 써라. 후자만 전제를 공격한다.
2. **리드의 주장을 인용해 넣는다.** 검토자가 무엇을 반박해야 하는지 알아야 반박한다.
   "나는 X 라고 판단했다 — 코드로 확인해라" 형태.
3. **완료 보고에 "리드 지시 중 틀린 것" 항목을 필수로 둔다.** 없으면 "없음" 이라고 쓰게 하고,
   그게 반복되면 검토가 작동하지 않는 것이다.
4. **아첨 금지를 프롬프트에 적는다.** "충분하다" 가 답이면 근거를 대라고 요구한다.

```
효과 없는 지시   "이 티켓을 검토해라"
효과 있는 지시   "내가 준 지시 중 틀린 것을 찾아라. 아첨하지 마라.
                 나는 A 라고 판단했다 — 코드로 확인해라. 확인 못 하면 판정불가라고 써라"
```

## 적용됐는지 확인하는 방법
**검토 한 라운드에서 리드의 전제가 몇 개 뒤집혔는지 센다.** 우리 최근 라운드에서는 6건이
뒤집혔다(예산 산술이 자기 임계를 넘음 · 프로세스 관리가 고아를 남김 · 기준선 숫자가 다른
모집단 · 판정 규칙이 사실상 자동통과 · 발췌 정책이 자기 가드를 무력화 · 존재하지 않는
CLI 플래그). 0 이 나오는 라운드가 이어지면 검토자를 바꿔라.
